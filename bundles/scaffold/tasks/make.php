...
class Scaffold_Make_Task {
	...
	public function run($arguments)
	{
		...
		$this->file    	  = Config::get('scaffold::scaffold.file');
		$this->migrate    = Config::get('scaffold::scaffold.migrate');
		$this->parser     = Config::get('scaffold::scaffold.parser');
		$this->templates  = Config::get('scaffold::scaffold.templates');
		...
		elseif(count($arguments) == 0)
		{
			...
			if (!File::exists($file))
			{
				$this->log('File '.$file. ' in ROOT directory does not exist.');die();
			}
	...
	public function make($arguments)
	{
		...
		else
		{
			...
			$this->create_view('exibir');
			$this->create_view('adicionar');
			$this->create_view('editar');
	...
	{
		$files = scandir(path('app').'controllers');

		$files = array_diff($files, array('.', '..','base.php', 'home.php'));

		$this->data['controllers'] = array_values($files);

		if(!file_exists(path('app').'views'.DS.'templates'.DS.'scaffold.blade'.EXT))
		{
			$this->create_view('template');
		}
	...
	public function create_view($view)
	{
		try
		{
			...
			if($view == 'template')
			{
				$path = path('app').'views'.DS.'templates'.DS;
	...
	{
		$controller = path('app').'controllers'.DS.Str::plural($argument).EXT;
		$model = path('app').'models'.DS.$argument.EXT;	
		$views = path('app').'views'.DS.Str::plural($argument);
		$migration = path('app').'migrations'.DS. '0000_00_00_000000_create_' .Str::plural($argument).'_table'.EXT;

		if (File::delete($controller))
		{
			$this->log('Controller removed.');
		}

		if (File::delete($model))
		{
			$this->log('Model removed.');
		}

		$mig = DB::table('laravel_migrations')->where('name', '=', '0000_00_00_000000_create_' .Str::plural($argument).'_table')->delete();
		if (File::delete($migration))
		{
			if ($mig) {
				$this->log('Migration removed.');
			}
		}

		if (Schema::drop(Str::plural($argument))) {
			$this->log('Table removed.');
		}

		if (is_dir($views)) {
			File::rmdir($views);
			$this->log('Views removed.');
		}
